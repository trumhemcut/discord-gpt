...
@bot.event
async def on_ready():
    await bot.change_presence(activity=discord.Game(name="Coded by Phi Huynh#9888"))
...
async def on_message(message):
    ...
    if allow_dm and not message.author.bot and not message.content.startswith(bot.command_prefix):
        ...
        try:
            ...
            response = nashbot.completion.response(message.content)
            if not response:
                response = "Sorry, I can't generate a response right now."

            # Comment here because of slow loading message to discord
            # token_count = 0
            # result = ""
...
async def welp(ctx):
    ...
    embed.add_field(
        name="!bonk", value="Clears history of the bot", inline=False)
    ...
    embed.add_field(name="!toggleactive",
                    value="Toggle the current channel to the list of active channels", inline=False)
    embed.add_field(name="!toggledm",
                    value="Toggle if DM should be active or not", inline=False)
